...
//Construtor
Rodada::Rodada(int indiceRodada, int tamanhoRodada){ 
...
void Rodada::RealizaRodada(FilaMM1& fila){
    while(fila.EstatisticasColetadas < this->Tamanho) {
			fila.TrataProximoEvento();
		}
}

//Função que coleta resultados da fila para a rodada
void Rodada::ColetaResultadosDaRodada(FilaMM1& fila){
    double tempoMedio = fila.TempoMedioDeEsperaNaFila();
    double varianciaTempoMedio = fila.VarianciaDoTempoDeEsperaNaFila();
    Escritor esc = Escritor();
    std::fstream arquivo;
